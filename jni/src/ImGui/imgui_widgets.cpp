...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // 计算滑块框架的宽度
    float slider_width = window->SizeFull.x * 0.29f;  //滑块宽度占窗口宽度的25%
    
    ImRect new_frame_bb = ImRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x + slider_width, frame_bb.Max.y)); 
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    const bool hovered = ItemHoverable(new_frame_bb, id, ImGuiItemFlags_None);
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive(id);
    if (!temp_input_is_active)
    {
    ...
    if (temp_input_is_active)
    {
        ...
        return TempInputScalar(new_frame_bb, id, label, data_type, p_data, format, is_clamp_input ? p_min : NULL, is_clamp_input ? p_max : NULL);
    }
    ...
    // 计算滑块值的百分比
    float percentage = (value - min_value) / (max_value - min_value);
    
    
        
    // 根据滑块的值确定填充矩形的宽度
    float fill_width = new_frame_bb.GetWidth() * percentage; // 确保 percentage 在 0.0 到 1.0 之间
    
    // 确定填充矩形的右边界位置
    float fill_max_x = new_frame_bb.Min.x + fill_width; // 计算填充矩形的右边界
    if (fill_max_x > new_frame_bb.Max.x) { // 如果填充矩形的右边界超出了框架的右边界
        fill_max_x = new_frame_bb.Max.x; // 将填充矩形的右边界限制在框架的右边界
    }
    
    // 使用提取的坐标来绘制框架
    RenderFrame(
        ImVec2(new_frame_bb.Min.x, new_frame_bb.Min.y + 15.f), 
        ImVec2(new_frame_bb.Max.x, new_frame_bb.Max.y - 15.f), 
        frame_col,
        true,
        g.Style.GrabRounding
    );
    
    // 使用框架颜色来绘制填充矩形
    window->DrawList->AddRectFilled(
        ImVec2(new_frame_bb.Min.x, new_frame_bb.Min.y + 15.f), 
        ImVec2(fill_max_x, new_frame_bb.Max.y - 15.f), 
        ImColor(147,189,64,255),
        g.Style.GrabRounding
    );

    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(new_frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    ImVec2 text_size = CalcTextSize(value_buf, value_buf_end);
    ImVec2 text_pos = ImVec2(grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x - text_size.x) / 2, grab_bb.Min.y + (grab_bb.Max.y - grab_bb.Min.y - text_size.y) / 2);
    
    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
    // 现在使用提取的坐标来绘制抓取手柄
    window->DrawList->AddRectFilled(
    ImVec2(grab_bb.Min.x - text_size.x/3.0f, grab_bb.Min.y + 8), 
    ImVec2(grab_bb.Max.x + text_size.x/3.0f, grab_bb.Max.y - 8), 
    ImColor(0, 0, 0, 0),
    style.GrabRounding
);
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");    
    // 绘制文本，确保文本颜色与抓取手柄背景颜色形成对比
    window->DrawList->AddText(NULL, g.FontSize, text_pos, ImColor(0, 0, 0, 0), value_buf, value_buf_end);
